...
public interface MemberRepository extends JpaRepository<Member, Long> {
  ...
  // 📌 [16] 소셜 ID와 가입 채널로 회원 조회 (카카오 로그인용)
  // 카카오 고유 ID (socialId)와 가입 채널("KAKAO")을 이용하여 회원을 찾습니다.
  // 이메일이 없을 경우에도 고유하게 회원을 식별할 수 있습니다.
  // @EntityGraph를 추가하여 역할(role) 정보도 함께 로드하도록 설정합니다.
  @EntityGraph(attributePaths = {"role"}, type = EntityGraph.EntityGraphType.LOAD)
  Optional<Member> findBySocialIdAndSignupChannel(String socialId, String signupChannel); // ✨ 이 한 줄만 추가!
}